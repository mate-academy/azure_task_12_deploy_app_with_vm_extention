...
Set-AzVMExtension `
    ...
    -VMName $vmName `
    -Location $location `
    -Name "installAppScript" `
    -Publisher "Microsoft.Azure.Extensions" `
    -ExtensionType "CustomScript" `
    -TypeHandlerVersion "2.1" `
    -SettingString "{ 'fileUris': ['$scriptUrl'], 'commandToExecute': 'bash install-app.sh' }"

Write-Host "Deployment completed. Access the application via http://$dnsLabel.uksouth.cloudapp.azure.com:8080"
